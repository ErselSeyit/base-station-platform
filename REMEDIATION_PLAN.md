...
|------|-----|-----|-------|-----|
| `service/diagnostic_service.py` | ~~2807~~ **386** | HIGH → **DONE** | one module = Flask app + TCP server + routing + orchestration + wire protocol | **Done, test-first (34 new characterisation tests):** split into 8 cohesive modules — `models.py`, `learning_engine.py`, `backends.py` (+RULES), `cloud_client.py`, `transport_adapters.py` (ProtocolAdapter/TCP/Serial/MQTT, tested via fake sockets), `optional_services.py` (the ~14 optional AI-subsystem imports + `*_AVAILABLE` flags + ERR_/ROUTE_ constants, shared to avoid a circular import), and `http_adapter.py` (Flask API, ~50 routes, tested via a real Flask test client: health/diagnose/HMAC-auth/learning-stats). `diagnostic_service.py` re-exports every name (importers + runtime entrypoint unchanged) and is now orchestration + `main` only. **2807 → 386 LOC (−86%)**; 187 ai-diagnostic tests green in CI. Next (optional): split `http_adapter.py`'s 50 routes into Flask blueprints per resource. |
| `service/bi_report_generator.py` | 1251 | HIGH → **PARTIALLY DONE** | report generation monolith | **Pure logic extracted, test-first:** the Site Verification pass/warn/fail acceptance (3GPP/Huawei KPI thresholds) moved to `ssv_status.py` + 7 characterisation tests; `_get_ssv_status` delegates and maps status→colour. The remainder is matplotlib PDF rendering (≈40 chart methods) — left by design; a full split would be by report section behind a shared renderer, gated on figure-level tests. |
| `virtual-basestation/mips_device.py` | 1158 | MEDIUM → **WON'T-DECOMPOSE (documented)** | simulator state + metrics + fault model + wire | **Deliberately single-file:** the container builds it with `COPY mips_device.py .` and runs `python mips_device.py` — a one-file deployment. Splitting into sibling modules would break that image without also changing the Dockerfile, for a dev/test simulator (not production service code). Kept whole on purpose; revisit only if it becomes a package. |
| `service/predictive_maintenance.py` | ~~912~~ **655** | MEDIUM → **DONE** | model + features + serving in one | **Done, test-first:** extracted `maintenance_models.py` (dataclasses) + `maintenance_analytics.py` (pure trend/regression, fan probability/status, confidence, recommendation text); service methods are thin delegators (call sites unchanged); +14 characterisation tests over the numeric logic; dead imports (numpy/statistics) removed. |
...
| `service/son_functions.py` | ~~752~~ **273** | MEDIUM → **DONE** | many SON functions in one file | **Done, test-first:** extracted `son_models.py` (enums + CellMetrics/SONRecommendation) and `son_optimizers.py` (MLB/MRO/CCO/EnergySaving optimizers); `son_functions.py` keeps SONEngine + module API and re-exports; +7 characterisation tests (MLB scenarios, serialisation, engine end-to-end); dead imports removed. |
| `service/drone_integration.py` | ~~741~~ **320** | MEDIUM → **DONE** | — | **Done, test-first:** extracted `drone_models.py` (enums + GeoPoint/Waypoint/FlightPath/CapturedData/DroneState/Mission) and `flight_path_planner.py` (pure orbit/spiral/grid geometry); service keeps DroneController + orchestration and re-exports; +9 characterisation tests (haversine distance, waypoint patterns/radius/counts); dead imports removed. |
